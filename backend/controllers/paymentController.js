...
exports.checkout = async (req, res) => {
  try {
    ...
    // Marca os itens como pagos (NÃO REMOVE MAIS)
    await db.query(
    ...
    // NÃO REMOVE MAIS ITENS PAGOS DO CARRINHO
    // await db.query('DELETE FROM cart_items WHERE cart_id = $1 AND paid = TRUE', [cart_id]);
    // await db.query('DELETE FROM cart_items WHERE cart_id = $1', [cart_id]);

    res.json({
      message: "Pagamento realizado! Jogos adicionados à biblioteca.",
    });
  } catch (err) {
    res
      .status(500)
      .json({ error: "Erro ao finalizar compra", details: err.message });
  }
};
